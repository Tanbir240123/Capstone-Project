...
uploaded_file = st.file_uploader("Upload Online Retail Excel file",type=["xlsx", "xls"])
...
if uploaded_file is not None:
    ...
    try:
        ...
        observed_revenue, predictions_dataset = predict_future_revenue(  df,lookahead_window)
        ...
        timeseries_graph = px.line(visualization_matrix,x="Date",y="Sales",color="Type",title="Sales Analysis: Historical vs Forecast")
        ...
        st.dataframe(predictions_dataset,use_container_width=True,hide_index=True)
        ...
        st.metric(label=f"Projected Sales - Upcoming {lookahead_window} Days",value=f"${aggregated_prediction:,.2f}")
        ...
        st.download_button(label="Export Sales Forecast",data=exported_bytestream,file_name="sales_forecast.csv",mime="text/csv")

    except Exception as processing_anomaly:

        st.error(f"Unable to complete sales forecasting: {processing_anomaly}")